...
function processRows(rows) {
  ...
  rows.forEach(row => {
    ...
    if (!name) return;
    items.push({
      id: DB.uid(), catId: cats[cn].id, name,
...
function addManual() {
  ...
  S.items.push({
    id: DB.uid(), catId: cat.id, name, price,
...
const DISH_STYLES = [
  {
    name: 'DARK LUXURY', ac: '#c9a84c',
    build() {
      const cur = getCur();
      return `<div style="background:#080808;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:7px">
        ${getSampleDishes().map(d => `
          <div style="background:#111;border-radius:9px;overflow:hidden;border:0.5px solid #2a2a2a">
            <div style="height:60px;background:#1a1a1a;display:flex;align-items:center;justify-content:center;font-size:20px">${d.ico}</div>
            <div style="padding:6px 8px">
              <div style="font-size:9px;font-weight:700;color:#c9a84c;margin-bottom:1px">${d.n}</div>
              <div style="font-family:'Bebas Neue',sans-serif;font-size:12px;color:#fff">${d.p} ${cur}</div>
            </div>
          </div>`).join('')}
      </div>`;
    }
  ...
  {
    name: 'POLAROID STACK', ac: '#16a34a',
    build() {
      const cur = getCur(); const rs = [-3, 2, -1.5, 3];
      return `<div style="background:#f0ede6;padding:14px 10px">
        <div style="display:flex;flex-wrap:wrap;gap:9px;justify-content:center">
          ${getSampleDishes().map((d, i) => `
            <div style="background:#fff;padding:5px 5px 24px;box-shadow:1px 3px 10px rgba(0,0,0,.12);transform:rotate(${rs[i]}deg);width:80px;position:relative">
              <div style="height:68px;background:#e8e0d0;display:flex;align-items:center;justify-content:center;font-size:20px">${d.ico}</div>
              <div style="position:absolute;bottom:3px;left:0;right:0;text-align:center;font-size:8px;color:#555;font-style:italic">
                ${d.n}<br><span style="color:#16a34a;font-weight:600;font-size:9px">${d.p} ${cur}</span>
              </div>
            </div>`).join('')}
        </div>
      </div>`;
  ...
  {
    name: 'DIAGONAL SLASH', ac: '#e11d48',
    build() {
      const cur = getCur();
      return `<div style="background:#fff;padding:10px;display:flex;flex-direction:column;gap:6px">
        ${getSampleDishes().map(d => `
          <div style="height:58px;border-radius:9px;overflow:hidden;display:flex;border:0.5px solid #f0f0f0;background:#fafafa">
            <div style="width:58px;flex-shrink:0;background:#111;display:flex;align-items:center;justify-content:center;font-size:18px;position:relative">
              <div style="position:absolute;top:0;right:-10px;bottom:0;width:20px;background:#fafafa;transform:skewX(-8deg)"></div>
              ${d.ico}
            </div>
            <div style="flex:1;padding:9px 11px">
              <div style="font-size:10px;font-weight:600;color:#111;margin-bottom:1px">${d.n}</div>
              <div style="font-size:9px;color:#999;margin-bottom:3px">${d.s || ''}</div>
              <div style="font-size:11px;font-weight:700;color:#e11d48">${d.p} ${cur}</div>
            </div>
          </div>`).join('')}
      </div>`;
    }
  ...
  {
    name: 'ARCH CARDS', ac: '#c9a84c',
    build() {
      const cur = getCur();
      return `<div style="background:#0a0800;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:7px">
        ${getSampleDishes().map(d => `
          <div style="background:#111;border-radius:10px;overflow:hidden;border:0.5px solid #2a2200">
            <div style="border-radius:0 0 999px 999px;overflow:hidden;height:70px;margin:0 6px;background:#1a1400;display:flex;align-items:center;justify-content:center;font-size:22px">${d.ico}</div>
            <div style="padding:6px;text-align:center">
              <div style="font-size:9px;font-weight:600;color:#c9a84c;margin-bottom:1px">${d.n}</div>
              <div style="font-family:'Bebas Neue',sans-serif;font-size:12px;color:#fff">${d.p} ${cur}</div>
            </div>
          </div>`).join('')}
      </div>`;
  ...
  {
    name: 'WAVE RIBBON', ac: '#ec4899',
    build() {
      const cur = getCur();
      return `<div style="background:#0f0014;padding:10px 0">
        ${getSampleDishes().map((d, i) => `
          <div style="height:52px;position:relative;overflow:hidden;${i > 0 ? 'margin-top:-6px' : ''}">
            <div style="position:absolute;inset:0;background:linear-gradient(90deg,rgba(15,0,20,.88),rgba(15,0,20,.2) 50%,rgba(15,0,20,.88));display:flex;align-items:center;padding:0 13px;justify-content:space-between;z-index:2">
              <div style="display:flex;align-items:center;gap:7px">
                <span style="font-size:16px">${d.ico}</span>
                <div style="font-size:10px;font-weight:600;color:#fff">${d.n}</div>
              </div>
              <div style="background:rgba(236,72,153,.18);border:0.5px solid rgba(236,72,153,.4);border-radius:20px;padding:2px 9px;font-size:9px;font-weight:700;color:#ec4899;white-space:nowrap">${d.p} ${cur}</div>
            </div>
          </div>`).join('')}
      </div>`;
    }
...
function getSampleDishes() {
  ...
  if (items.length >= 4) {
    return items.map(it => ({
      n:   it.name.split(' ').slice(0, 2).join(' '),
      p:   it.price,
      ico: S.cats.find(c => c.id === it.catId)?.ico || '🍽️',
      s:   it.desc?.slice(0, 20) || '',
    }));
  }
  return [
    { n: 'شيش طاووق', p: '55', ico: '🍗', s: 'دجاج مشوي' },
    { n: 'سلطة سيزر', p: '35', ico: '🥗', s: 'خس طازج' },
    { n: 'مارغريتا',  p: '58', ico: '🍕', s: 'موتزاريلا' },
    { n: 'كريم برولي',p: '38', ico: '🍮', s: 'فانيليا' },
  ];
}
...
async function runDishGen() {
  ...
  const ld   = document.getElementById('gen-loader');
  const fill = document.getElementById('gen-fill');
  const gt   = document.getElementById('gen-txt');
  ld.style.display = 'flex';
  const msgs = [...GEN_MSGS].sort(() => Math.random() - .5);
  let prog = 0;
  const iv = setInterval(() => {
    prog += Math.random() * 14 + 6; if (prog > 92) prog = 92;
    fill.style.width = prog + '%';
    gt.textContent = msgs[Math.min(Math.floor(prog / 25), msgs.length - 1)];
  }, 110);
  ...
  div.innerHTML = `
    <div class="gen-item-head">
      <span class="gen-item-name" style="color:${ds.ac}">${ds.name}</span>
      <button class="gen-del-btn" onclick="this.closest('.gen-item').remove()" aria-label="حذف">
...
function saveMenu() {
  ...
  const menu = {
    id, slug, name,
    sub:      document.getElementById('r-sub').value.trim(),
    ico:      document.getElementById('r-ico').value.trim() || '🍽️',
    currency: getCur(),
    desc:     document.getElementById('r-desc').value.trim(),
    style: S.style, color: S.color, colorDk: S.colorDk,
    cats: S.cats, items: S.items,
...
function copyLink() {
  navigator.clipboard?.writeText(document.getElementById('link-url-txt').textContent)
    .then(() => toast('📋 تم نسخ الرابط'));
}

function autoSlug() {
  const n = document.getElementById('r-name').value.trim();
  if (n) document.getElementById('r-slug').value = DB.slug(n);
}
...
function renderMenusList() {
  const menus = DB.all().reverse();
  const w = document.getElementById('menus-list');
  if (!menus.length) {
    w.innerHTML = '<div class="empty-state"><i class="ti ti-layout-grid"></i><p>لا توجد منيوهات</p><small>احفظ منيو أولاً</small></div>';
    return;
  }
  w.innerHTML = menus.map(m => `
    <div class="menu-list-item">
      <div class="mli-thumb" style="background:${m.color || '#1a3a28'}">${m.ico || '🍽️'}</div>
      <div class="mli-info">
        <div class="mli-name">${m.name}</div>
        <div class="mli-meta">${(m.items || []).length} طبق · ${(m.cats || []).length} فئة</div>
      </div>
      <div class="mli-actions">
        <button class="mla" onclick="editMenu('${m.id}')" aria-label="تعديل ${m.name}"><i class="ti ti-edit"></i></button>
        <button class="mla primary" onclick="previewMenu('${m.id}')" aria-label="معاينة"><i class="ti ti-eye"></i></button>
        <button class="mla" onclick="copyMenuLink('${m.slug}')" aria-label="نسخ رابط"><i class="ti ti-link"></i></button>
        <button class="mla" onclick="openMenu('${m.slug}')" aria-label="فتح المنيو"><i class="ti ti-external-link"></i></button>
        <button class="mla" onclick="delMenu('${m.id}')" aria-label="حذف" style="color:#E24B4A"><i class="ti ti-trash"></i></button>
      </div>
    </div>`).join('');
}
...
function updateStats() {
  const s = DB.stats();
  document.getElementById('st-m').textContent = s.m;
  document.getElementById('st-i').textContent = s.i;
  document.getElementById('st-c').textContent = s.c;
}
...
function editMenu(id) {
  ...
  document.getElementById('r-sub').value   = m.sub   || '';
  document.getElementById('r-ico').value   = m.ico   || '';
  document.getElementById('r-slug').value  = m.slug  || '';
  document.getElementById('r-desc').value  = m.desc  || '';
  document.getElementById('r-cur').value   = m.currency || 'ر.س';
  showPage('info', document.querySelector('.nav-item'));
  renderItemsTable(); syncPhone();
  toast('✏️ تحميل للتعديل');
}

function previewMenu(id) {
  const m = DB.get(id); if (!m) return;
  S.cats = m.cats||[]; S.items = m.items||[];
  S.color = m.color||'#1B7040'; S.colorDk = m.colorDk||'#0F4424'; S.imgs = DB.mImgs(id)||{};
  document.getElementById('r-name').value = m.name;
  document.getElementById('r-sub').value  = m.sub  || '';
  document.getElementById('r-ico').value  = m.ico  || '🍽️';
  document.getElementById('r-cur').value  = m.currency || 'ر.س';
  document.getElementById('r-desc').value = m.desc || '';
...
function delMenu(id) {
  if (!confirm('حذف المنيو نهائياً؟')) return;
  DB.del(id); renderMenusList(); updateStats();
  toast('🗑 تم الحذف');
}

function copyMenuLink(slug) {
  navigator.clipboard?.writeText(`menucraft.app/m/${slug}`).then(() => toast('📋 تم نسخ الرابط'));
}

function openMenu(slug) {
  window.open(`menu.html?slug=${slug}`, '_blank');
}
...
function dlTpl() {
  const data = [
    ...
    ['المقبلات','شيش طاووق مشوي','55','دجاج طري متبّل بالثوم','75','الأكثر طلباً'],
    ['المقبلات','سبرنج رول الدجاج','38','معجنات هشة محشوة بالدجاج','',''],
    ['السلطات','سلطة سيزر الكلاسيكية','35','خس روماني وصوص سيزر','45','كلاسيك'],
    ['البيتزا','مارغريتا كلاسيكية','58','صوص طماطم وموتزاريلا','','كلاسيك'],
    ['البيتزا','بيبروني الحار','68','بيبروني وفلفل حار','85','حار 🌶️'],
    ['الحلويات','كريم برولي','38','كاسترد فانيليا بقشرة كراميل','','مميز'],
  ];
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(data);
  ws['!cols'] = [{ wch: 14 },{ wch: 22 },{ wch: 8 },{ wch: 30 },{ wch: 10 },{ wch: 14 }];
  XLSX.utils.book_append_sheet(wb, ws, 'Menu');
  XLSX.writeFile(wb, 'menucraft-template.xlsx');
...
function defaultCats() {
  return [
    { id: 'dc1', name: 'المقبلات', ico: '🍖', bg: '#1a3a28' },
    { id: 'dc2', name: 'السلطات',  ico: '🥗', bg: '#1a3020' },
    { id: 'dc3', name: 'البيتزا',  ico: '🍕', bg: '#2e1c08' },
    { id: 'dc4', name: 'الحلويات', ico: '🍮', bg: '#2a2200' },
  ];
}
...
function renderPhoneHome() {
  ...
  document.getElementById('phone-content').innerHTML = `
    <div style="background:${bg};min-height:440px">
      <div class="pm-topbar" style="background:${bg1};border-bottom-color:${br}">
        <div class="pm-ibtn" style="background:${bg2};border-color:${br};color:${txM}"><i class="ti ti-menu-2"></i></div>
        <div style="text-align:center">
          <div class="pm-brand" style="color:${txP}">${name.toUpperCase().slice(0,12)}</div>
          <div class="pm-sub" style="color:${col}">${sub.toUpperCase().slice(0,20)}</div>
        </div>
        <div class="pm-ibtn" style="background:${bg2};border-color:${br};color:${txM}"><i class="ti ti-search"></i></div>
      </div>
      <div class="pm-hero" style="background:${cd}">
        <div class="pm-hero-bg"></div>
        <div class="pm-hero-inner">
          <div>
            <div class="pm-hero-badge">SPECIAL OFFERS</div>
            <div class="pm-hero-title">${name.toUpperCase().slice(0,12)}</div>
            <div class="pm-hero-sub">${desc.slice(0, 28)}</div>
          </div>
          <div class="pm-hero-orb">${ico}</div>
        </div>
      </div>
      <div class="pm-sec">
  ...
  document.getElementById('phone-content').innerHTML = `
    <div style="background:${bg};min-height:440px;padding-bottom:16px">
      <div class="pm-items-bar" style="background:${isLight?'rgba(245,245,245,.95)':'rgba(6,6,10,.95)'};border-bottom-color:${br}">
        <button class="pm-back" onclick="renderPhoneHome()" style="background:${isLight?'#e5e5e5':'#141419'};border-color:${br}">
          <i class="ti ti-arrow-right"></i>
        </button>
        <div>
          <div class="pm-cat-label" style="color:${txP}">${cat.name.toUpperCase()}</div>
          <div class="pm-cat-sub">${items.length} أطباق</div>
        </div>
      </div>
          ...
          <div class="pm-item" style="background:${bg2};border-color:${br}">
            ...
            <div class="pm-ibody">
              ...
              <div class="pm-ifoot">
                ...
                <button class="pm-iadd" style="background:${col}" onclick="addToCart('${it.id}',this)">
...
function renderPhoneCart() {
  document.getElementById('phone-content').innerHTML = `
    <div style="background:#06060A;min-height:440px">
      <div class="pm-items-bar">
        <button class="pm-back" onclick="renderPhoneHome()"><i class="ti ti-arrow-right"></i></button>
        <div>
          <div class="pm-cat-label">سلة الطلب</div>
          <div class="pm-cat-sub">${CART.reduce((s,c) => s+c.qty, 0)} عنصر</div>
        </div>
      </div>
      ${CART.length ? renderCartSection()
...
function renderCartSection() {
  ...
  return `<div class="pm-cart">
    <div class="pm-cart-title">
      سلة الطلب
      <span style="font-size:10px;color:${col};font-family:var(--font-body)">${CART.reduce((s,c)=>s+c.qty,0)} عنصر</span>
    </div>
    ${CART.map((c, i) => `
      <div class="pm-cart-row">
        <div class="pm-cn">${c.name}</div>
        <div class="pm-cq">
          <button class="pm-cqb" style="background:#2a2a2a" onclick="chQty(${i},-1)">−</button>
          <span style="font-size:10px;font-weight:700;min-width:12px;text-align:center">${c.qty}</span>
          <button class="pm-cqb" style="background:${col}" onclick="chQty(${i},1)">+</button>
        </div>
        <span class="pm-cp" style="color:${col}">${Math.round(c.price * c.qty)} ${cur}</span>
      </div>`).join('')}
    <div class="pm-cart-total">
      <span style="font-size:10px;color:var(--tx2)">المجموع</span>
      <span style="font-family:var(--font-display);font-size:16px;color:${col}">${Math.round(total)} ${cur}</span>
    </div>
...
function chQty(i, d) {
  ...
  if (phoneCat) renderPhoneItems(phoneCat); else renderPhoneCart();
}
...
function updateClock() {
  const n = new Date();
  const el = document.getElementById('psb-time');
